...
class newp_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'newp_rp', // Base ID
			__('Newp Recent Posts with Thumbnails', 'newp'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'newp' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'newp' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'newp' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:','newp' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php esc_html_e( 'No. of Posts:', 'newp' ); ?></label>
...
add_action( 'widgets_init', 'register_newp_widget' );  
function register_newp_widget() {  
    register_widget( 'newp_Recent_Posts' );  
}  
